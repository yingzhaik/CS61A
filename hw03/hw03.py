...
def count_change(total):
    ...
    def helper(n, coin):
        ...
        return helper(n-coin, coin) + helper(n, coin // 2) #第一个是找了一个面值为coin的硬币，然后继续用这个面值的硬币找钱；第二个是不使用这个免职的硬币找钱，选择coin//2面值的硬币找钱。

    def get_max_coin(n):
        def find_power(p):
            if p * 2 > n:
                return p
            return find_power(p * 2)
        return find_power(1)

    if total == 0:
        return 0
    return helper(total, get_max_coin(total))
